	...
	double precision scpr_pond(ndatmax,ndatmax)
      ...
      call getarg (5, tresnc)!		nro de modos-6
      read(tresnc, *) tresn
      ndat=tresn+6
      call getarg (6, outfile_1)!	rtdo del zeta value 
      call getarg (7, outfile_2)!	rtdo del nd value 
      call getarg (8, outfile_3)!	eigenvalues del grammiano
      call getarg (9, outfile_4)!	eigenvector del grammiano
...
!			------------------ 	ACA EMPIEZA A ARMAR LA MTZ PROYECCION    -------------
	...
	write(99,*) scpr2(i,j)
            scpr_pond(i,j)= scpr2(i,j) * dsqrt(mode_pond(j)) ! pondero segun ambos subespacios
            scpr2(i,j)=scpr_pond(i,j) * dsqrt(mode_pond2(i))
...
!el promedio de los eigenvalues indica la similutd de los espacios vibracionales (de 0 a 1, sin ponderar). 
