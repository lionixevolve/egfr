...
C de el RMSD  
      ...
      INTEGER i,j, site(500), site_ctr, ierr, man
      INTEGER i2,j2,ii,kk2,k1,k2,k,kk, str_len1, str_len2
      ...
      character*30 infile_a, infile_b, outfile_a, outfile_b, site_file
      character*30 out_rmsd_total, out_rmsd, out_vec
      character*5 str_lenc1, str_lenc2

C****LA Kb ESTA EN ANGSTROMS CUADRADO

      call getarg(1, infile_a)!					toda esta parte es p/ nombrar a los archivos (hasta...
      call getarg(2, infile_b)
      call getarg(3, site_file)
      call getarg(4, outfile_a)
      call getarg(5, outfile_b)
      call getarg(6, out_rmsd_total)
      call getarg(7, out_rmsd)
      call getarg(8, out_vec)

      	icont=0
	site_ctr=0
	open(15, file= site_file)
	do
        	site_ctr= site_ctr + 1
        	read (15, 99, iostat= ierr) site(site_ctr)
        	if ( ierr/= 0 ) exit
	enddo
	site_ctr= site_ctr - 1
	
      OPEN(21, FILE= infile_a)
      OPEN(11, FILE='cardA')
      OPEN(25, FILE='dim')
      do i=1,icontmax
...
200   continue
      ...
      if(icont.ne.icont2) write(99,*)"distinto nro de residuos"
...
C***************************************************
      open(50,file=out_rmsd_total) 
      open(51,file=out_vec)
...
C***************************************************
C Calculo RMSD de site
C***************************************************
      open(66,file=out_rmsd) 
      rmsdp=0.0d0

     	do j=1, site_ctr
         dist=0.0d0
		man=site(j)! 			esto no era necesario, pero estaba debuggeando y lo hice por si las moscas
         do kk=1,3
!            x=r1(j,kk)-r2(j,kk)
		x=r1(man,kk)-r2(man,kk)
            x2=x*x
            dist=dist+x2
         enddo
         rmsdp=rmsdp+dist/dfloat(site_ctr)
      enddo
