	...
	double precision scpr_pond(ndatmax,ndatmax)
      ...
      call getarg (5, tresnc)!		Nro de los modos.
      read(tresnc, *) tresn
      ndat=tresn+6
...
!			------------------ 	ACA EMPIEZA A ARMAR LA MTZ PROYECCION    -------------
      ...
      do i=1,pnumber2
         do j=1,pnumber
            ...
            ! pondero segun ambos subespacios
      scpr2(i,j)=scpr2(i,j) * dsqrt(mode_pond2(i)) * dsqrt(mode_pond(j))
         enddo!							
      enddo

      do i=1,pnumber2
        write(99, 91) (scpr2(i,j), j = 1, pnumber)
      enddo
